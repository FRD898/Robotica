...
def cinematica_inversa_RR_Newton(Xdeseado, Qinicial, L1, L2, max_iter=100, epsilon=1e-4):
...
def test_Newton():
    ...
    print("Coordenada x,y obtenida:", np.round(RR.cinematica_directa_RR(q[0], q[1], L1, L2), 4))
    #showError(e)


# Función que realiza el cálculo de la cinemática inversa (calculado usando el método de Newton)
def cinematica_inversa_RR_Gradiente(Xdeseado, Qinicial, L1, L2, alfa=0.5, max_iter=100, epsilon=1e-4):
    # Es importante usar "copy" para no sobrescribir el valor original
    q = Qinicial.copy()
    # Almacenamiento del error
    ee = []
...
def testGradiente():
    ...
    # Cinemática Inversa
    q, e = cinematica_inversa_RR_Gradiente(xd, qinit, L1, L2, 0.5, max_iteraciones, epsilon)
...
def main():
    print("Probando método de newton")
    test_Newton()
    print("Probando método de la gradiente")
    testGradiente()
